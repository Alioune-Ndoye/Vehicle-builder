    ...
    inquirer
      ...
      .then((answers: { vehicleToTow: Truck | Car | Motorbike; }) => {
        ...
        if (answers.vehicleToTow === truck) {
          console.log("A vehicle can't tow itself!");
          this.performActions();
        } else {
          if (answers.vehicleToTow instanceof Truck) {
            // If the selected vehicle is a Truck, log a message indicating it can't tow another truck
            console.log("A truck cannot tow another truck.");
          } else {
            // Otherwise, proceed with the towing operation
    ...
    inquirer
      ...
      .then((answers: { action: string; }) => {
        ...
        if (answers.action === 'Print details') {
          ...
          for (let i = 0; i < this.vehicles.length; i++) {
            if (this.vehicles[i].vin === this.selectedVehicleVin) {
              if (this.vehicles[i] instanceof Truck) {
                const selectedTruck = this.vehicles[i] as Truck;
                this.findVehicleToTow(selectedTruck);
                return; // Return to avoid immediate recursive call
              } else {
          ...
          // Find the selected vehicle and check if it's a motorbike
          for (let i = 0; i < this.vehicles.length; i++) {
            if (this.vehicles[i].vin === this.selectedVehicleVin) {
